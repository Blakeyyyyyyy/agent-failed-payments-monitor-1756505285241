...
// Create or update the Failed Payments table in Airtable
async function ensureFailedPaymentsTable() {
  try {
    // Try to access the table - if it doesn't exist, we'll create records structure
    const table = airtable('Failed Payments');
    addLog('Failed Payments table accessed successfully');
    return table;
  } catch (error) {
    addLog('Failed Payments table may not exist, will create records with proper fields');
    return airtable('Failed Payments');
  }
}
...
async function addToAirtable(paymentData) {
  try {
    const table = await ensureFailedPaymentsTable();
...
app.listen(port, () => {
  addLog(`Failed Payments Monitor started on port ${port}`);
});
